...
- Unity で一度プロジェクトを開き、`Library/PackageCache` に `com.unity.xr.meta-openxr@2.4.0`（または使用しているバージョン）がダウンロードされていること。
...
パッチの内容は Meta OpenXR 2.4.0 をベースに、パススルー初期化処理を `PassthroughInitializationGate` で順序制御しつつ診断ログを追加するものです。
